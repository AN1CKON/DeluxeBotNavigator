...
def register_review_settings_handlers(dp):
    ...
    async def change_cooldown_callback(callback: types.CallbackQuery, state: FSMContext):
        ...
        request_msg = await callback.message.answer(
            "⏰ Введите новое значение cooldown в минутах (0 - отключить, 1-1440):"
        )
    ...
    async def handle_cooldown_value(message: types.Message, state: FSMContext):
        ...
        try:
            ...
            if value < 0 or value > 1440:
                error_msg = await message.answer("❌ Значение должно быть от 0 до 1440 минут\n0 - отключить cooldown")
            ...
            error_msg = await message.answer("❌ Введите число от 0 до 1440")
            await asyncio.sleep(2)
...
<b>Выберите параметр для изменения:</b>"""
    ...
    keyboard = InlineKeyboardMarkup(
        inline_keyboard=[
            [InlineKeyboardButton(text="⏰ Изменить cooldown", callback_data="change_cooldown")],
            [InlineKeyboardButton(text="📊 Изменить дневной лимит", callback_data="change_daily_limit")],
            [InlineKeyboardButton(text="🔄 Сбросить к умолчанию", callback_data="reset_settings")],
