...
def register_admin_edit(dp):
    ...
    @dp.callback_query(F.data == "admin_move_mode")
    async def admin_move_mode_start(callback: types.CallbackQuery, state: FSMContext):
        """Переход в режим изменения порядка кнопок"""
        from ..edit_actions.move_action import show_move_menu
        await safe_delete_message(callback.message)
        await state.clear()
        await show_move_menu(callback.message, state)
