"""Главное меню редакти    # Определяем наличие изображения
    has_image = menu_item[6] and menu_item[6].strip()  # проверяем что image_path не None и не пустая строка
    has_image_text = "✅ Есть" if has_image else "❌ Нет"ния кнопки"""

from aiogram import types
from aiogram.fsm.context import FSMContext
from ...database.db import get_menu_item, get_menu_items
...
async def show_edit_button_menu(message: types.Message, item_id: int, state: FSMContext):
    ...
    has_image_exists = menu_item[9] and menu_item[9].strip()  # image_path находится под индексом 9
    ...
    keyboard = types.InlineKeyboardMarkup(inline_keyboard=[
        ...
        [
            types.InlineKeyboardButton(text=BUTTON_EDIT_MOVE, callback_data=f"edit_action_move:{item_id}"),
